...
module UIEvent = {
  ...
  module Wheel = {
    ...
    [@bs.obj] external make: (~on_wheel: Dom.wheelEvent => unit=?, unit) => t;
  };
...
module Dialog = {
  ...
  [@bs.obj]
  external make:
    (~on_cancel: Dom.event => unit=?, ~on_close: Dom.event => unit=?, unit) =>
    t;
};
...
module Details = {
  ...
  [@bs.obj] external make: (~on_toggle: Dom.event => unit=?, unit) => t;
};
