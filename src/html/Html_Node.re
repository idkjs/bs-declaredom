...
module ContentCategory = {
  ...
  type flow('a) = [ standard_flow | custom('a)];
  ...
  type phrasing('a) = [ standard_phrasing | custom('a)];
  type embedded = [
    audio
    | canvas
    | em
  ...
  type palpable('a) = [ standard_palpable | custom('a)];
  ...
  type content('a) = [ element('a) | other | flex_item];
...
external to_text_node: t([> text] as 'a) => Dom.text = "%identity";
external to_fragment: t([> fragment] as 'a) => Dom.documentFragment =
  "%identity";
external to_node: t([< content(_)]) => Dom.node = "%identity";
external to_element: t([< element(_)]) => Dom.element = "%identity";

external from_node: Dom.node => t([< content(_)]) = "%identity";

let show_element = element =>
  element |> to_element |> Webapi.Dom.Element.outerHTML;

let show_text = text_node => text_node |> to_text_node |> Webapi.Dom.Text.data;

[@bs.val]
external nodeList_to_array: Dom.nodeList => array(Dom.node) =
...
let rec show_fragment = fragment =>
  ...
  |> nodeList_to_array
  |> Js.Array.map(e => show @@ from_node(e))
  |> Js.Array.joinWith("\n")

and show = node =>
  switch (node |> to_node |> Webapi.Dom.Node.nodeType) {
...
let show_doc = (html: t(html)) => "<!DOCTYPE html>\n" ++ show(html);
